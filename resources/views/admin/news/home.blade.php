...
<div class="content-header">
  ...
  <div class="container-fluid">
      <div class="add_news text-right">
       <a href="{{url('add/news/'.$info_news->id) }}" class="btn btn-sm btn-success cust_color" style="margin: 11px;"><i class="fa fa-plus"></i> Add {{$info_news->title}}</a>
       </div>
    <div class="row mb-2">
				...
				<table class="table dataTable table-hover cust_table" id="table_id" style="margin-top: 5px !important;">
					...
					<tbody>
						...
						<tr>
                            ...
                            @else
							<td><img style="width:75px; height:60px;" src="{{url('/assets/images/news/'.$row->image)}}"></td>
							@endif
							<td>{{$row->title}}</td>
							...
							<td class="text-center">
								<div class="actions-btns dule-btns">
									<a href="{{url('/news/edit/'.$row->id .'/'.$info_news->id)}}" class="back_color btn btn-sm btn-info"><i class="fa fa-pencil"></i></a>
								</div>
							</td>
						</tr>
						@endforeach
